...
import { Menu, X, Facebook, Twitter, Send, Phone, Mail, ChevronDown } from "lucide-react";
...
export function PortalLayout() {
  ...
  const MAIN_NAV: NavItem[] = useMemo(() => {
    const directorates = directory.filter((d) => d.category === "Directorates");
    ...
    return [
      ...
      },
      {
        ...
        children: directorates.map((d) => ({
          to: "/directory",
          label: label(d),
          search: { type: "directorates", name: d.name },
        })),
      },
      ...
      <div className="hidden bg-primary text-primary-foreground md:block">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-2 text-xs">
          <div className="flex items-center gap-5">
            <span className="inline-flex items-center gap-1.5"><Phone className="h-3 w-3" /> 033-666-0577</span>
            <span className="inline-flex items-center gap-1.5"><Mail className="h-3 w-3" /> info@afarudcb.gov.et</span>
          </div>
          <div className="flex items-center gap-3 text-primary-foreground/80">
            <a href="#" aria-label="Facebook" className="hover:text-gold"><Facebook className="h-3.5 w-3.5" /></a>
            <a href="#" aria-label="Twitter" className="hover:text-gold"><Twitter className="h-3.5 w-3.5" /></a>
            <a href="#" aria-label="Telegram" className="hover:text-gold"><Send className="h-3.5 w-3.5" /></a>
      ...
      <header className="sticky top-0 z-40 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
        <div className="mx-auto grid max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 md:px-6">
          <Link to="/" className="flex min-w-0 items-center gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-primary/5 ring-1 ring-primary/10 shadow-soft overflow-hidden">
              <img src={logo} alt="Afar UDCB logo" className="h-full w-full object-cover" />
            </div>
            <div className="min-w-0">
              <div className="truncate font-display text-sm font-bold leading-tight text-foreground sm:text-base">
                {lang === "am" ? "የአፋር ክልል መንግስት" : "Afar Regional Government"}
              </div>
              <div className="truncate text-xs text-muted-foreground">
                {lang === "am" ? "የከተማ ልማት እና ግንባታ ቢሮ" : "Urban Development & Construction Bureau"}
              </div>
            </div>
          </Link>
          ...
          <nav className="col-span-2 hidden lg:col-span-1 lg:block">
            <ul className="flex items-center gap-1">
              {MAIN_NAV.map((item) => {
                ...
                return (
                  <li key={item.label} className={hasChildren ? "group relative" : undefined}>
                    <Link
                      to={item.to}
                      className={
    ...
    <footer className="mt-16 bg-sidebar text-sidebar-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 md:grid-cols-4">
        <div>
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 place-items-center rounded-xl bg-sidebar-accent overflow-hidden">
              <img src={logo} alt="Afar UDCB logo" className="h-10 w-10 object-contain" />
            </div>
            <div>
              <div className="font-display text-sm font-semibold">Afar Regional Government</div>
              <div className="text-xs text-sidebar-foreground/70">UDCB</div>
            </div>
          </div>
        ...
        <div>
          <h4 className="font-display text-sm font-semibold text-gold">{t("quickLinks")}</h4>
        ...
        <div>
          <h4 className="font-display text-sm font-semibold text-gold">{t("contact")}</h4>
          <ul className="mt-3 space-y-2 text-sm text-sidebar-foreground/80">
            <li className="flex items-start gap-2"><Phone className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold" /> 033-666-0577</li>
            <li className="flex items-start gap-2"><Phone className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold" /> 033-666-0576</li>
            <li className="flex items-start gap-2"><Mail className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gold" /> info@afarudcb.gov.et</li>
          </ul>
        </div>

        <div>
          <h4 className="font-display text-sm font-semibold text-gold">{t("follow")}</h4>
          <div className="mt-3 flex gap-2">
            <a href="#" aria-label="Facebook" className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent hover:bg-gold hover:text-gold-foreground"><Facebook className="h-4 w-4" /></a>
            <a href="#" aria-label="Twitter" className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent hover:bg-gold hover:text-gold-foreground"><Twitter className="h-4 w-4" /></a>
            <a href="#" aria-label="Telegram" className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent hover:bg-gold hover:text-gold-foreground"><Send className="h-4 w-4" /></a>
          </div>
        </div>
      </div>
      <div className="border-t border-sidebar-border">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-6 py-4 text-xs text-sidebar-foreground/60 md:flex-row">
          <div>© {new Date().getFullYear()} Afar UDCB. All rights reserved.</div>
          <div>Designed and Developed by Skylink Technologies</div>
        </div>
      </div>
    </footer>
  );
  ...
  return (
    <section className="relative border-b overflow-hidden">
      <div
        ...
        aria-hidden
      />
      <div className="absolute inset-0 bg-gradient-to-r from-primary/45 via-primary/25 to-primary/10" aria-hidden />
      <div className="relative mx-auto max-w-7xl px-4 py-12 text-primary-foreground sm:px-6 md:py-20">
        {eyebrow && (
          <div className="inline-flex items-center gap-2 rounded-full bg-primary-foreground/15 px-3 py-1 text-xs font-medium tracking-wide text-gold ring-1 ring-primary-foreground/25 backdrop-blur">
            <ChevronDown className="h-3 w-3 rotate-[-90deg]" /> {eyebrow}
          </div>
        )}
        <h1 className="mt-3 font-display text-2xl font-bold tracking-tight drop-shadow-md sm:text-3xl md:text-5xl">{title}</h1>
        {description && <p className="mt-3 max-w-2xl text-primary-foreground/95 drop-shadow sm:text-base md:text-lg">{description}</p>}
      </div>
    </section>
  );
}
